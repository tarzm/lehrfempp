...
// ======================================================================
  ...
  // clang-format off
  std::array<std::array<double, 2>, 6> node_coord{
...
for (size_type level = 0; level < L; ++level) {
    ...
    //get the edges' numbering
    // for(auto edge : poly_mesh_ptr->Entities(1)){
    //     int point_idx = 0;
    //     for(auto point : edge->SubEntities(1)){
    //         std::cout << "\n Point " << point_idx << " of edge " << poly_mesh_ptr->Index(*edge) << " has index " << poly_mesh_ptr->Index(*point);
    //         point_idx++;
    //     }
    // }
