            ...
            for (auto edge : cell->SubEntities(1)){
                ...
                if (boundary_d_edge_(*edge) || boundary_minus_edge_(*edge)){
                    ...
                    for(int basis_test = 0; basis_test < n_basis; basis_test++){
                        ...
                        for (int i = 0; i < gram_dets_s.size(); i++){
                            if (b[i].dot(normal) < 0){
                                rhs_vec[dofs_cell[basis_test]] -= (b[i].dot(normal)) * gD_evaluated[i]
        ...
        for (const lf::mesh::Entity *edge : dgfe_space_ptr_->Mesh()->Entities(1)){
            ...
            if (boundary_d_edge_(*edge)){
                ...
                for(int basis_test = 0; basis_test < n_basis; basis_test++){
                    ...
                    for (int i = 0; i < gram_dets_s.size(); i++){
                        ...
                        rhs_vec[dofs_cell[basis_test]] -= gD_evaluated[i] * ( (a_coeff_(cell, zeta_box_s.col(i))[0] *  nabla_v).dot(normal)
            ...
            if (boundary_n_edge_(*edge)){
                ...
                for(int basis_test = 0; basis_test < n_basis; basis_test++){
                    //sum over qr points
                    for (int i = 0; i < gram_dets_s.size(); i++){
                        rhs_vec[dofs_cell[basis_test]] += gN_evaluated[i] * legendre_basis(basis_test, max_legendre_degree_, zeta_box_s.col(i))
                                                            * w_ref_s[i] * gram_dets_s[i];
                    }
                }
            }
