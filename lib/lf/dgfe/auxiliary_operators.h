...
template<typename SCALAR, typename MESH_FUNCTION, typename DGFEMESHFUNC>
L2ProjectionSqrtAGradBasis<SCALAR, DGFEMESHFUNC> l2_proj_diffusion(std::shared_ptr<const lf::dgfe::DGFESpace> dgfe_space_ptr, MESH_FUNCTION m_a_coeff, unsigned max_integration_degree){
    ...
    dim_0_vec.reserve(n_cells * n_local_dofs);
    dim_1_vec.reserve(n_cells * n_local_dofs);
